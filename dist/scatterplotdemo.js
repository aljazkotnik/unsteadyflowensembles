(function () {
  ...
  */
  ...
  var template_circle = "\n<circle r=\"5\" fill=\"cornflowerblue\"></circle>\n"; // template_point

  var template_path = "\n<path r=\"5\" fill=\"none\" stroke=\"gainsboro\" stroke-width=\"4\"></path>\n"; // template_point
  ...
  var unsteadyScatterData = /*#__PURE__*/function () {
    function unsteadyScatterData(d) {
      _classCallCheck(this, unsteadyScatterData);

      this.t = 0; // 'd' is the name of the file that holds the filenames for the available unsteady values.

      var obj = this;
      obj.attached = false;
      obj.node_current = svg2element$1(template_circle);
      obj.node_outline = svg2element$1(template_path);
      obj.node_current.onmouseenter = obj.highlight.bind(obj);
      obj.node_current.onmouseleave = obj.unhighlight.bind(obj);
      obj.node_outline.onmouseenter = obj.highlight.bind(obj);
      obj.node_outline.onmouseleave = obj.unhighlight.bind(obj); // Maybe the data should be reformatted, and the points should be held together? That's how it's done for the unsteady line plots? Anyway, this loading can be resolved later.
      // But don't I ultimately want to follow this specific approach? Just keep all the variables separate for loading? And only load them when absolutely necessary? However, this object still needs an x and a y variable...
      // Note that blockage, eff_lost etc are just calculated for the outlet plane. They could be calculated at any other plane also. In that sense they are also slices.
      // For the line plots the x and y are prescribed. Don't allow the user to change this for now. Just keeep it here.

      obj.promises = ["blockage_unst", "eff_lost_unst"].map(function (variable) {
        return fetch(d[variable]).then(function (res) {
          return res.json();
        });
      }); // The outline promise here is a hack....

      obj.timesteps = [];
      obj.outlinepromise = Promise.all(obj.promises).then(function (data) {
        // data now has eff_lost_unst and blockage_unst. They need to be changed from column data to row data. Maybe it's more convenient to just store it as row data in the first place?
        ...
        obj.outline = obj.timesteps;
      }); // then
    } // constructor


    _createClass$1(unsteadyScatterData, [{
      key: "current",
      get: function get() {
      ...
      value: function update(xaxis, yaxis) {
        ...
        if (obj.timesteps.length > 0) {
          ...
          obj.node_current.setAttribute("cx", xaxis.scale(obj.current.x));
          obj.node_current.setAttribute("cy", yaxis.scale(obj.current.y));
        } // if
      ...
      value: function highlight() {
        ...
        obj.node_current.setAttribute("fill", "orange");
        obj.node_current.parentNode.appendChild(obj.node_current);
        obj.node_outline.setAttribute("stroke", "gray");
      ...
      value: function unhighlight() {
        var obj = this;
        obj.node_current.setAttribute("fill", "cornflowerblue");
        obj.node_outline.setAttribute("stroke", "gainsboro");
      } // unhighlight

    }]);

    return unsteadyScatterData;
  }(); // unsteadyScatterData
  ...
  var unsteadyLinePlot = /*#__PURE__*/function (_dbsliceCrossfilterPl) {
    ...
    function unsteadyLinePlot(tasks, configobj) {
      ...
      obj.node.querySelector("input.card-title").setAttribute("value", "Outlet" + (tasks[0] ? ": " + tasks[0].taskId : "")); // Append the plot backbone.
      ...
      obj.promises = obj.tasks.map(function (m) {
        var promise = fetch(m.unsteady_scatterplot).then(function (res) {
          return res.json();
        }).then(function (json) {
          var g = new unsteadyScatterData(json);
  ...
  var StackableGroup = /*#__PURE__*/function () {
    function StackableGroup(members) {
      ...
      /*
      obj.plot.svgobj.xmenu.variables = [{name: "x", axistype: "linear", extent: [0.288,0.44]}];
      obj.plot.svgobj.ymenu.variables = [{name: "entropy", axistype: "linear", extent: [1140,1260]}];
...
//# sourceMappingURL=scatterplotdemo.js.map
